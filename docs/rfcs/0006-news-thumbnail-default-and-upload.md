...
- **가공 완료** — §4-1-1 참조:
  ...
  | **번들본** | **240 × 240** | **32,489 B** (−96.3%) |

  표시칸 78px 기준 3배 밀도(234px)를 덮는다.

### 4-1-1. 로고만 투명배경으로 (2026-07-28, 팀장님 지시)

초안은 원본을 그대로 `Jimp.cover(240,240)`로 줄여 넣었다. 그 결과 **밝은 회색 배경이 이미지에 구워진 채** 들어가 다크 모드에서도 밝은 사각형으로 보였고, 아래쪽 "머니로드" 워드마크까지 78px 칸에 눌려 들어갔다.

**팀장님 지시**: 배경을 투명으로, **로고 마크만**(아래 글씨 제외), 정사각 칸의 **완전 정중앙**에.

먼저 기존 에셋 재활용을 검토했으나 `android-icon-foreground.png`는 **흰색 로고**라 밝은 배경에서 보이지 않아 쓸 수 없었다. 원본에서 직접 추출했다.

**측정값** (Jimp로 원본 스캔):

```
배경색           #F3F4F8
로고 원          중심 (626.5, 474.0)  지름 718px
"머니로드" 글씨   y 919 ~ 1093        ← 원(y ≤ 831)과 완전히 분리 → 크롭으로 제거
```

**추출 방법** — 단순 색상 제거(배경색과 같은 픽셀을 투명화)는 쓸 수 없다. 원 안의 **흰 화살표(#FFFFFF)와 배경(#F3F4F8)이 너무 가까워** 화살표까지 반투명해지기 때문이다. 그래서 반지름 기준으로 나눴다:

1. 원의 외접 정사각형으로 크롭 (글씨 자동 제외)
2. **반지름 92% 안쪽은 알파를 건드리지 않는다** — 흰 화살표 보호
3. 92% 바깥에서만 배경색과의 색거리로 알파 계산 — 원본의 안티에일리어싱을 그대로 살려 테두리가 매끄럽다
4. 캔버스의 70% 지름으로 축소해 240×240 투명 캔버스 정중앙에 합성

**검증** (출력 파일 재스캔):

| 항목 | 값 |
|---|---|
| 네 모서리 알파 | `0` `0` `0` `0` (완전 투명) |
| 여백 좌·우·상·하 | **36 / 36 / 36 / 36** — 정중앙 |
| 흰 화살표 알파 | 255 (선명) |
| 원 테두리 알파 | 147 (안티에일리어싱 유지) |
| 완전 투명 비율 | 51.0% |

**부수 효과**: 배경이 투명해져 **다크 모드 문제가 함께 해소**됐다. 초안이 남긴 우려(밝은 사각형)가 사라지고, 칸 배경은 `NewsThumb`의 `t.bgSubtle`이 테마에 따라 그린다. → **Q15**
...
| **Q15** | §4-1-1 — 로고가 투명해지면서 칸 배경은 `NewsThumb`의 `backgroundColor: t.bgSubtle`이 그린다. 이대로 **연한 회색 타일 위에 로고**로 둘까, 아니면 기본 이미지일 때는 배경도 없애 **카드 배경 위에 로고만 떠 있게** 할까? *(실제 썸네일에는 로딩 자리표시 역할이 있어 `t.bgSubtle`을 유지해야 한다)* | 타일 유지 — 실제 썸네일과 칸 모양이 같아 목록이 흔들리지 않는다 |
